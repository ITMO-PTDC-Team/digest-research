...
type TDigestImpl struct {
	...
	IntegratedQCalculator func(float64,float64) float64
	IntegratedLocationCalculator func(float64, float64) float64
}

func (o *TDigestImpl) GetName() string {
	return o.Name
}

func NewTDigestFactory(name string, compression float64, integratedQFunc func(float64,float64) float64, integratedLocationFunc func(float64,float64) float64) TDigest {
	t := &TDigestImpl{
		Name:                     name,
		Compression:              compression,
		IntegratedQCalculator:      integratedQFunc,
		IntegratedLocationCalculator: integratedLocationFunc,
	}
...
func (o *TDigestImpl) New() *TDigestImpl {
	return o.NewWithCompression(1000)
}

// NewWithCompression initializes a new distribution with custom compression.
func (o *TDigestImpl) NewWithCompression(c float64) *TDigestImpl {
	t := &TDigestImpl{
		Compression: c,
	}
	t.maxProcessed = processedSize(0, t.Compression)
...
func (t *TDigestImpl) AddCentroid(c Centroid) {
...
func (t *TDigestImpl) Merge(other *TDigest) {
    if other == nil {
        return
    }
    otherImpl, ok := (*other).(*TDigestImpl)
    if !ok {
		otherImpl.Centroids(nil)
    }
    otherImpl.process()
    t.AddCentroidList(otherImpl.processed)
}

func (t *TDigestImpl) process() {
...
func (t *TDigestImpl) Centroids(cl CentroidList) CentroidList {
	t.process()
	return append(cl, t.processed...)
}

func (t *TDigestImpl) Count() float64 {
	t.process()
...
func (t *TDigestImpl) updateCumulative() {
...
func (t *TDigestImpl) CDF(x float64) float64 {
	t.process()
	t.updateCumulative()
...
func (t *TDigestImpl) integratedQ(k float64) float64 {
	return t.IntegratedQCalculator(k, t.Compression)
}

func (t *TDigestImpl) integratedLocation(q float64) float64 {
	return t.IntegratedLocationCalculator(q, t.Compression)
}
