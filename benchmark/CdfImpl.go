...
func CompareCDF(cdf, td CDF, quantiles []float64) []string {
	var results []string
	for _, q := range quantiles {
		cdfValue := cdf.Quantile(q)
		tdValue := td.Quantile(q)
		results = append(results, fmt.Sprintf("Quantile: %.8f, CDF: %.8f, TDigest: %.8f, Result: %.8f", q, cdfValue, tdValue,(tdValue-cdfValue)/cdfValue))
	}
	return results
}