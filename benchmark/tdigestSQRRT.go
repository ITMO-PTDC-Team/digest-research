...
func (t *TDigest) integratedQ(k float64) float64 {
	return math.Pow((math.Sin(math.Min(k, t.Compression)*math.Pi/t.Compression-math.Pi/2.0) + 1.0) / 2.0,1.0/2.5)
}

func (t *TDigest) integratedLocation(q float64) float64 {
	return t.Compression * (math.Asin(2.0*math.Pow(q,2.5)-1.0) + math.Pi/2.0) / math.Pi
}
